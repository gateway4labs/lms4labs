...
/// If data submitted, process and store in database
	...
	$msg= new stdClass();
	 ...
	 if ($DB->insert_record('wlab_list',$msg)){
       
		$conditions = array("name" => $msg->name, "description" => $msg->description, "accesstype" =>$form->accesstype);
		$lab=$DB->get_record('wlab_list', $conditions);
		$codconnect = array("id" => $msg->accesstype);
		$typeaccess=$DB->get_record('wlab_access', $codconnect);
		//redirect to the type of connection
		if (stripos($typeaccess->name,"deusto")){
			$url="Location: connectiondeusto.php?id=". $lab->id;
			header( $url ) ;
		}
		else {
			$url="Location: connectioniLab.php?id=". $lab->id;
			header( $url ) ;
...
<tr valign="top">
    <td align="left"><b>access:</b></td>
    	<td align="left">
	<select name="accesstype" id="accesstype" value="<?php  p($form->accesstype) ?>">
